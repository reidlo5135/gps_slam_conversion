...
class ConverterNode(Node):
    ...
    def __position_test(self) -> None:
        ...
        std_point_slam_x1: int = std_point_slam_x1;
        std_point_slam_y1: int = std_point_slam_y1;
        test_pos_1: PositionPoint = PositionPoint(std_point_slam_x1, std_point_slam_y1);

        std_point_slam_x2: int = std_point_slam_x2;
        std_point_slam_y2: int = std_point_slam_y2;
        test_pos_2: PositionPoint = PositionPoint(std_point_slam_x2, std_point_slam_y2);

        slam_map_width: int = slam_map_width;
        slam_map_height: int = slam_map_height;
        slam_map_resoultion: float = slam_map_resoultion;
        
        std_point_lon1: float = std_point_lon1;
        std_point_lat1: float = std_point_lat1;

        std_point_lon2: float = std_point_lon2;
        std_point_lat2: float = std_point_lat2;
        ...
        self.get_logger().info(f"===== {CONVERTER_NODE} position_test [1] =====");
        gps_pos_test_1: PositionPoint = self.__position_converter.convert_slam_to_gps(test_pos_1.x, test_pos_1.y);
        slam_pos_test_1: PositionPoint = self.__position_converter.convert_gps_to_slam(gps_pos_test_1.x, gps_pos_test_1.y);

        gps_node_test_1: PositionPoint = PositionPoint(127.124136, 37.465932);
        gps_differ_x_1: float = abs(std_point_lon1- gps_pos_test_1.x);
        gps_differ_y_1: float = abs(std_point_lat1 - gps_pos_test_1.y);

        self.get_logger().info(f"{CONVERTER_NODE} GPS\n\tlon : [{gps_pos_test_1.x}]\n\tlat : [{gps_pos_test_1.y}]");
        self.get_logger().info(f"{CONVERTER_NODE} GPS Differ\n\tlon : [{gps_differ_x_1}]\n\tlat : [{gps_differ_y_1}]");
        self.get_logger().info(f"{CONVERTER_NODE} SLAM\n\tx : [{round(slam_pos_test_1.x * 100) / 100.0}]\n\ty : [{round(slam_pos_test_1.y * 100) / 100.0}]");

        self.get_logger().info(f"===== {CONVERTER_NODE} position_test [2] =====");
        gps_pos_test_2: PositionPoint = self.__position_converter.convert_slam_to_gps(test_pos_2.x, test_pos_2.y);
        slam_pos_test_2: PositionPoint = self.__position_converter.convert_gps_to_slam(gps_pos_test_2.x, gps_pos_test_2.y);

        gps_node_test_2: PositionPoint = PositionPoint(127.124107, 37.465798);
        ...
        # self.get_logger().info(f"===== {CONVERTER_NODE} position_test [3] =====");
        # gps_pos_test_3: PositionPoint = self.__position_converter.convert_slam_to_gps(test_pos_3.x, test_pos_3.y);
        # slam_pos_test_3: PositionPoint = self.__position_converter.convert_gps_to_slam(gps_pos_test_3.x, gps_pos_test_3.y);
